...
const Shorts = () => {
  ...
  // Normalize and filter shorts only
  const shortProducts: Product[] = allProducts
    .filter((product: any) => product.category?.toLowerCase() === "shorts")
    .map((p: any) => ({
      ...p,
  ...
  }, [shortProducts, sortBy]);

  // Grid class based on view mode
  const getGridClass = () => {
    if (viewMode === 'list') return 'grid grid-cols-3 gap-6';
    if (viewMode === 'grid-5') return 'grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6';
    ...
    return (
      <div className="w-full bg-white">
        ...
        <div className="px-10 py-20 text-center">
          ...
          <p className="mt-4 text-gray-600">Loading shorts...</p>
        </div>
      </div>
    );
  }
        ...
        {sortedProducts.length > 0 ? (
          ...
          <div className="col-span-full text-center py-20">
            ...
            <p className="text-gray-500">
              Check back soon for new shorts in our collection.
            </p>
          </div>
        )}
      </div>
